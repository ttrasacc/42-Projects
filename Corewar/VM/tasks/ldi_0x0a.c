...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/01 11:07:40 by ttrasacc          #+#    #+#             */
/*   Updated: 2017/12/01 11:07:42 by ttrasacc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	extra_lines(t_process *p, int dest)
{
	...
	{
		p->reg[p->params[2]] =\
		(int)g_vm[(p->addr + p->c + (dest % IDX_MOD)) % VM_SIZE] << 24;
		p->reg[p->params[2]] +=\
		(int)g_vm[(p->addr + p->c + 1 + (dest % IDX_MOD)) % VM_SIZE] << 16;
		p->reg[p->params[2]] +=\
		(int)g_vm[(p->addr + p->c + 2 + (dest % IDX_MOD)) % VM_SIZE] << 8;
		p->reg[p->params[2]] +=\
		(int)g_vm[(p->addr + p->c + 3 + (dest % IDX_MOD)) % VM_SIZE];
	}
}

void		load_indirect(t_process *p)
{
	int dest;
	int addr;
